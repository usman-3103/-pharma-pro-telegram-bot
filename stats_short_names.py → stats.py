...
def _display_source(source: str) -> str:
    """Short admin-only label. Stored source values are never changed."""
    labels = {
        "Аптеки Турции": "Аптеки Турции",
        "MAX — Турецкая аптека": "Турецкая аптека",
        "MAX - Турецкая аптека": "Турецкая аптека",
        "MAX — Лекарства из Турции купить": "Лекарства · MAX",
        "MAX - Лекарства из Турции купить": "Лекарства · MAX",
        "Лекарства из Турции — MAX": "Лекарства · MAX",
        "Лекарства из Турции - MAX": "Лекарства · MAX",
...
def _format_period(connection: sqlite3.Connection, title: str, period: str) -> list[str]:
    ...
    if entrance_sources:
        ...
        for source, count in visible:
            lines.append(f"• {_display_source(source)} — {count}")
